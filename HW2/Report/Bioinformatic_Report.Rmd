...
In this section we want to focus on graph indices. Starting from the adjacency matrices obtained applying a PDC measure to signals coming from EO and EC states, it is possible to create a directed graph and compute some global and local measures. When we refer to global indices we just consider the average of the local measure evaluated in each node. In order to compute all the analysis we used the library `networkx` in Python, created to efficiently deal with graphs. 
...
A motif is a configuration of $s$ nodes, that shows up in the graph a significant amount of times. More precisely, the idea behind the detection of motifs, is to understand whether the pattern in the graph is due to randomness, or it's a recurrent pattern that describes something in the phenomena we are interested in. Given a graph $G$ and a configuration of nodes $G_k$ we want to state that $G_k$ is a motif and we need to generate $\tilde{G} = \{\tilde{G_1},...,\tilde{G_N}\}$, that is a set of $N$ similar random graphs to $G$, i.e. all the nodes of each graph in $\tilde{G}$ must have the same degree of the original graph.
Once obtained $\tilde{G}$, let's set the following quantities:

- $f_{G}(G_k)$ = # of occurrencies of $G_k$ in $G$
- $f'_{G}(G_k)$ = # of occurrencies of $G_k$ in $G$ that involves different nodes among them
- $f_{\tilde{G}}(G_k)$ = average # of occurrencies of $G_k$ in $\tilde{G}$

And the user must decide the following parameters [in square brackets the one chosen for this analysis]:

- $s$ = Size of the motif [3 and 4]
- $N$ = Number of random graphs [1000]
- $p$ = Probability of over-representation [0.01]
- $U$ = Uniqueness threshold [4]
- $M$ = Ratio of over-representation [1.1]

In order to define $G_k$ as a motif, the following must hold:

- $P(f_{\tilde{G}}(G_k) > f_{G}(G_k)) < p$
- $f'_{G}(G_k) > U$
- $f_{G}(G_k) > M \cdot f_{\tilde{G}}(G_k)$

Meanwhile, if we want to define an anti-motif, the following must hold:

- $P(f_{\tilde{G}}(G_k) < f_{G}(G_k)) > 1-p$
- $f_{G}(G_k) < f_{\tilde{G}}(G_k)/M$

**Size 3 Motifs**

In order to detect the presence of motifs, we have exploited `mFinder`, a tool designed for this purpose by the Weizmann Institute of Science. The results are in Table 9 and Table 10. For the Motif Dictionary, please refer to the relative folder of this part of the work. In general the only difference from the two rest-states, is in the presence of the motif 238 in the EC state. The motif 238 represent the complete clique of 3 nodes with edges in both directions (since it is a directed graph). Since we are taking into account the frequency related to the Alpha waves, the greater connection in the EC state makes perfectly sense.
...
This approach has been proposed in 2008 by Rosvall and Bergstrom. The idea behind is that in a community there are interdependent interactions among the users. For instance, an interaction between user A and user B, influence the interaction between user B and user C. The main goal is no more to understand the physical structure in the network, but how the flow of the information can run between components of the network, and group users that share this flow. To solve the problem of finding the optimal clustering of a graph, the idea is to perform a random walk in the network and find a description of minimum information. In **Figure 10** we find the results of the Infomap algorithm applied on the brain networks.
...
![Adjacency matrices](Fig1.png)

![Adjacency matrices with different densities](Fig2.png)


![Graphs with 20% density](Fig3.png)


![Reduced graphs](Fig4.png)




![Alternative frequency (30 Hz)](Fig5.png)

![Global indices with differen densities](Fig6.png)


![Local indices graphs](Fig7.png)

![Anti-motif 36](Fig8.png)
